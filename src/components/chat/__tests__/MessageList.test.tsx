...
// Mock the ToolCallLabel component
vi.mock("../ToolCallLabel", () => ({
  ToolCallLabel: ({ toolInvocation }: { toolInvocation: { toolName: string; args: Record<string, string> } }) => (
    <div data-testid="tool-call-label">{toolInvocation.args?.command} {toolInvocation.args?.path}</div>
  ),
}));
...
test("MessageList renders messages with parts", () => {
  ...
  expect(screen.getByText("create /App.jsx")).toBeDefined();
});
